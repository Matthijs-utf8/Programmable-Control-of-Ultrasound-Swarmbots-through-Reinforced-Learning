...
processed_csv = pd.read_csv(training_data_folder + filename_dynamics_csv + ".csv")
del processed_csv["Unnamed: 0"]
processed_csv = processed_csv[processed_csv["Magnitude"] <= 20]
...
def create_vector_fields(csv0):
    ...
    for piezo in range(4):
        ...
        # vect_fields = np.vstack( (vect_fields, np.stack( (np.tan(v_interp / u_interp), magn_interp), axis=2)[np.newaxis, :, :, :] ) )
        vect_fields = np.vstack( (vect_fields, np.stack((u_interp, v_interp), axis=2)[np.newaxis, :, :, :]))

    return vect_fields

# np.save('Vector_fields.npy', create_vector_fields(csv0=processed_csv))
...
# plot_velocity_field(csv=processed_csv, piezo=1)
# feature_x = "Vpp"
# feature_y = "Magnitude"
# fig = colorplot_2d(csv=processed_csv,
#              feature_x=feature_x,
#              feature_y=feature_y,
#              normalize_x=True,
#              normalize_y=False,
...
# cv2.imwrite(f"{feature_x}_vs_{feature_y}.png", fig)
